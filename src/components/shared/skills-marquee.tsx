...
export function SkillsMarquee({ variant = "default", items }: Props) {
  ...
  return (
    <div
      aria-hidden
      className="marquee-mask marquee-pause-hover relative left-1/2 right-1/2 -mx-[50vw] w-screen overflow-hidden border-y border-white/5 bg-[#0b1315] py-3.5 dark:bg-[#0b1315]"
    >
...
function MarqueeTrack({ list }: { list: string[] }) {
  return (
    <ul className="flex items-center font-mono text-[11px] font-semibold uppercase tracking-[0.28em] text-teal-300">
      {list.map((name, i) => (
        <li
          key={`${name}-${i}`}
